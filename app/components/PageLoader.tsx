...
export default function PageLoader({
  ...
  useEffect(() => {
    setMounted(true);
    // Use a persistent, shared host to avoid container add/remove races
    const target =
      (document.getElementById("page-loader-host") as HTMLElement | null) ||
      (document.getElementById("portal-root") as HTMLElement | null) ||
      document.body;
    setContainer(target);
  }, []);
  ...
  // Portal to static container. If not mounted yet (SSR/first render), render nothing — `#initial-loader` covers screen until hydration.
  return mounted && container ? createPortal(overlay, container) : null;
}
